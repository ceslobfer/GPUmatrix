...
substantial changes of the code. Another problem of these packages is
...
TODO: Explain the hard part: intall CUDA 11.3 or 11.0 depending on torch
or tensorflow, install torch and/or tensorflow, install GPUMatrix.
...
## 3.2 Matrix factorizations and linear system solving

TODO: include svd, qr, … TODO: lu not yet implemented
...
|    `eigen`    |                 `eigen(Gm)`                 |    `base`     |
|     `svd`     |                  `svd(Gm)`                  |    `base`     |
|    `ginv`     | `ginv(Gm, tol = sqrt(.Machine$double.eps))` |    `base`     |
|    `chol`     |                 `chol(Gm)`                  |    `base`     |
...
GPUMatrix also deals with sparse matrices. Let’s see an example.
...
We can convert it into a gpu.matrix as follows,

``` r
Ag <- gpu.matrix(A)
```

A (and Ag) can be multiplied by a dense matrix B:

``` r
B <- matrix(rnorm(10*3),10,3)
Bg <- gpu.matrix(B)
...
Torch (or Tensorflow) have not implemented (yet) the multiplications of
two sparse matrices. GPUmatrix automatically casts the smaller one to
dense and gives a warning. The result is also dense.
...
```
    ...
    ## Warning in warningSparseTensor_torch(y): Not allowed with sparse matrix, matrix
    ## will be cast to dense for the operation. May cause memory problems
...
operations.](GPUmatrix_CPU.png)

## 5.2 Using GPUMatrix with Tensorflow

TODO: strongly recommended to use Torch.
