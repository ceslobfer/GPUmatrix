...
creationGPUmatrix_all <- function(refMatrix){
  ...
  #float32 cpu
  GPUm_f32_cpu <- gpu.matrix(refMatrix, dtype = "float64",device = "cpu")
  #float64 cuda sparse
...
plotTimeComparison_TwoMatrix<- function(nrowInterval=c(500,700,1000,1400,2000,2800,4000),
  ...
  for(interval in c(1:length(nrowInterval))){
    ...
    # A <- matrix(data, nrow = nrows, ncol = ncols)
    listMatrixComparison1 <- creationGPUmatrix_all(A1)
    listMatrixComparison2 <- creationGPUmatrix_all(A2)
    timeRes <- TwoFunctionTimeCalculation(listMatrixComparison1[typeMatrixPlot],
                                          listMatrixComparison2[typeMatrixPlot],
                                          f ,nrows, ncols,Time)
    resTable <- cbind(timeRes,
                      rep(nrows,length(typeMatrixPlot)),
                      typeMatrixPlot)
...
drawPlotFunction <- function(DataFrameTimes,namePlot){
  ...
  p<-ggplot(data=DataFrameTimes, aes(x=Size, y=Time, group=Type)) +
    ...
    theme_classic()+
    scale_color_manual(values = c("#14690A","#1B06B4", "#1B06B4","#DE044A","#DE044A"))+
