...
subtitle: "Seamlessly harness the power of GPU compu-ting in R"
...
-   **MUST INSTALL:**
    ...
    -   CUDA Toolkit 11.3. Link [here](https://developer.nvidia.com/cuda-11.3.0-download-archive).
...
TODO: Explain the hard part: intall CUDA 11.3 or 11.0 depending on torch or tensorflow, install torch and/or tensorflow, install GPUMatrix.
...
Torch (or Tensorflow) have not implemented (yet) the multiplications of two sparse matrices. GPUmatrix automatically casts the smaller one to dense and gives a warning. The result is also dense.
...
![Computation time for GPU and R-base CPU for different operations. Time is in seconds and Size=n where matrix is *(n x n)* dimension*.*](Rplot.png)
...
![Computation time for GPUMatrix on CPU and MKL-R for different operations. Time is in seconds and Size=n where matrix is *(n x n)* dimension. There is a substantial speed performance in element-wise operations.](GPUmatrix_CPU.png)
